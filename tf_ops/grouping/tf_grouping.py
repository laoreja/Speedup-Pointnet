...
#def assign_idx(nsample, true_idx, end_idx):
#    '''
#    Input:
#        true_idx: 1-D array, 3rd column of tf.where(dist_lt_radius)
#        (dist_lt_radius: [batch_size, npoint, ndataset])
#        end_idx: [batch_size, npoint], each batch's certain centroids'
#        last true_idx's idx
#        nsample: int
#    '''
#    return grouping_module.assign_idx(nsample, true_idx, end_idx) # TODO
#ops.NoGradient('AssignIdx')
def assign_idx(nsample, true_mat):
    '''
    Input:
        true_idx: 1-D array, 3rd column of tf.where(dist_lt_radius)
        (dist_lt_radius: [batch_size, npoint, ndataset])
        ...
        nsample: int
    '''
    return grouping_module.assign_idx(true_mat, nsample) # TODO
ops.NoGradient('AssignIdx')

def query_square_point(grid_size, nsample, all_xyz, centroids_xyz, limits, sizes):
    '''
    Input:
    ...
    return grouping_module.query_square_point(all_xyz, centroids_xyz, limits, sizes, grid_size, nsample)
ops.NoGradient('QuerySquarePoint')
